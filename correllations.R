...
inflam_cdr <- function(df) {
  ...
  sapply(markers, get_cdr_mmse_corr)

  get_ptau_csf_corr <- function(csf_marker) {
    ptau_coef <- washington |>
      lm(as.formula(paste0(csf_marker, " ~ ",
         paste("mean_ptau217",
         "age",
         "female", sep = " + "))),
      data = _) |>
  ...
  csf_markers <- washington |> select(starts_with("CSF_")) |> names()
  sapply(csf_markers, get_ptau_csf_corr)
  ...
  sapply(c("zscore",
           "raw_suvr",
           "centiloid"), function(outcome) {
    coefs <- lm(as.formula(paste0(outcome,
                                  " ~ ",
                                  paste(
                                        "mean_ptau217",
                                        "age",
